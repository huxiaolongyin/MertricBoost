...
from metricboost.models.system import APILog, Log, LogType, User
...
class APILoggerMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        if all(
            [declude not in path for declude in SETTINGS.ADD_LOG_ORIGINS_DECLUDE]
        ) and (
            "*" in SETTINGS.ADD_LOG_ORIGINS_INCLUDE
            or any([include in path for include in SETTINGS.ADD_LOG_ORIGINS_INCLUDE])
        ):
...
class APILoggerAddResponseMiddleware(BaseMiddleware):
    """
