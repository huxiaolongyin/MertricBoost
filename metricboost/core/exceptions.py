...
def BaseHandle(
    exc: Exception, handle_exc, code: int | str, msg: str, status_code: int = 500
) -> JSONResponse:
    if isinstance(exc, handle_exc):
        return JSONResponse(
            content=dict(code=str(code), msg=msg), status_code=status_code
        )
    else:
        return JSONResponse(
            content=dict(code=str(code), msg=f"Exception handler Error, exc: {exc}"),
...
async def IntegrityHandle(req: Request, exc: Exception) -> JSONResponse:
    return BaseHandle(
        ...
        IntegrityError,
        500,
        f"IntegrityError，{exc}, path: {req.path_params}, query: {req.query_params}",
        500,
    )
...
async def RequestValidationHandle(_: Request, exc: Exception) -> JSONResponse:
    return BaseHandle(
        exc, RequestValidationError, 422, f"RequestValidationError, {exc}"
    )


async def ResponseValidationHandle(_: Request, exc: Exception) -> JSONResponse:
    return BaseHandle(
        exc, ResponseValidationError, 422, f"ResponseValidationError, {exc}"
    )
