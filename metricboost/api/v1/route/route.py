...
from metricboost.core.dependency import DependAuth
from metricboost.core.response import Success
from metricboost.models.system import Menu, Role, User
    ...
    for menu in menus:
        ...
        if menu.parent_id == parent_id:
            ...
            if simple:
                ...
                if menu.component:
                    menu_dict["meta"]["layout"] = menu.component.split("$", maxsplit=1)[
                        0
                    ]
...
@router.get("/getConstantRoutes", summary="查看常量路由(公共路由)")
async def _():
    """
    查看常量路由
    :return:
...
@router.get("/getUserRoutes", summary="查看用户路由菜单", dependencies=[DependAuth])
async def _():
    """
    ...
    if is_super:
        role_routes: list[Menu] = await Menu.filter(
            constant=False
        )  # todo 处理隐藏菜单是否需要返回
    else:
