...
@router.get("/{id}", summary="获取指标发布分享详情")
async def _(id: int):
...
@router.patch("/{id}", summary="更新指标发布分享")
...
@router.delete("/{id}", summary="删除指标发布分享")
async def _(id: int):
