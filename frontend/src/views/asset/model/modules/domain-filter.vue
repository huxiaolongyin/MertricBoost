...
import { fetchDeleteDomain, fetchGetDataDomainList, fetchGetTopicDomainList } from '@/service/api';
...
// 和父组件双向绑定模型
const model = defineModel<Api.SystemManage.DataModelSearchParams>('model', {
  required: true
});
  ...
  return [
    {
      ...
      key: 'dataDomain',
      children:
        dataDomainData?.records?.map(item => ({
    ...
    },
    {
      ...
      key: 'topicDomain',
      children:
        topicDomainData?.records?.map(item => ({
          label: item.domainName,
...
const handleDelete = async (type: DomainType, value: number) => {
  // 处理删除逻辑
  let error;

  if (type === 'dataDomain') {
    ({ error } = await fetchDeleteDomain({ id: value }));
  } else {
    ({ error } = await fetchDeleteDomain({ id: value }));
  }
    ...
    <NFlex justify="space-between">
      ...
      {node.option.domainType && (
        <NFlex justify="end" size={0}>
          <NButton size="tiny" onClick={() => handleEdit(node.option)} quaternary>
...
const updateCheckedKeys = (
  ...
  model.value.domainIds = [];
  options.forEach(option => {
    if (!option) return;
    if (option.domainType === 'dataDomain' && option.value !== null) {
      model.value.domainIds?.push(option.value as number);
