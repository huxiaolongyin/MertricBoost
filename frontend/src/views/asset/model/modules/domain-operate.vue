...
import { fetchAddDataDomain, fetchAddTopicDomain, fetchUpdateDomain } from '@/service/api';
...
const handleSubmit = async () => {
  ...
  const operationMap: Record<string, (data: any) => Promise<any>> = {
    ...
    'edit-dataDomain': fetchUpdateDomain,
    'edit-topicDomain': fetchUpdateDomain
  };
...
<template>
  <NModal v-model:show="visible" display-directive="show" transform-origin="center" class="h-3/4">
    <NCard :title="modalTitle" :bordered="false" size="small" class="h-3/4 w-3/6 card-wrapper">
      ...
      <NFlex justify="end">
        <NButton type="primary" class="mb-5 w-20" @click="handleSubmit">提 交</NButton>
      </NFlex>
    </NCard>
  </NModal>
</template>
