...
<template>
  <NCard style="width: 800px" title="添加入参定义" :bordered="false" size="huge" role="dialog" aria-modal="true">
    ...
    <NForm :model="formValue" :rules="rules">
      ...
      <NFormItem label="是否必填" label-placement="left" :label-width="80" path="isRequired">
        <NSwitch v-model:value="formValue.isRequired" />
      </NFormItem>
      <NFormItem label="示例值" label-placement="left" :label-width="80" path="example">
        <NInput v-model:value="formValue.example" placeholder="将API提供给别人时的参考值" />
      </NFormItem>
      <NFormItem label="描述" label-placement="left" :label-width="80" path="paramDesc">
        <NInput v-model:value="formValue.paramDesc" placeholder="请输入入参描述" type="textarea" />
      </NFormItem>
    </NForm>
